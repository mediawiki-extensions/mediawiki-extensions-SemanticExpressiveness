...
window.semanticExpresiveness.ui.ContextPopup.instances = 0;
window.semanticExpresiveness.ui.ContextPopup.prototype = {
	/**
	 * @const
	 * @enum number
	 */
	 ...
	 * the popup.
	 * @param time integer
	 * @param callback Function executed after this.show() (optional)
	 */
	 ...
	 * @param time integer
	 * @param callback Function executed after this.close() (optional)
	 */
	 ...
	 * @param time integer
	 * @param callback Function exectued for each affected popup after this.close() (optional)
	 */
	 ...
	 * @param orientation Object containing values from this.ORIENTATION for the properties 'horizontal'
	...
	_draw_doPositioning: function() {
		...
		var viewPortEndeX = $( window ).scrollLeft() + $( window ).width();
		var xOverflow = popupEndX - viewPortEndeX;
		var origPosX = posX;
		var xPointerMarging = 0;
		if( xOverflow > 0 ) {
			posX -= xOverflow;
		...
		var pointerEndX = xPointerMarging + divPointer.position().left + divPointer.width();
	 ...
	 * @param divPopup jQuery the old popup
	...
	*/
	 ...
	 * @param popup jQuery dom element required for additional positioning calculation
	 ...
	 * because the parents subject would not be existant if hidden. Therefore the parent can only be
	 * returned as long as the parents content still exists within the DOM.
	 * @return ContextPopup|null
	 */
	getParent: function() {
